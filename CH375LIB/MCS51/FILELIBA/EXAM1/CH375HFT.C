...
/* 读取原文件 */
		printf( "Open\n" );
		mCopyCodeStringToXRAM( mCmdParam.Open.mPathName, "\\C51\\CH375HFT.C" );  /* 文件名,该文件在C51子目录下 */
...
/* 列出文件 */
			...
			for ( c = 0; c < 255; c ++ ) {  /* 最多搜索前255个文件 */
				i = mCopyCodeStringToXRAM( mCmdParam.Open.mPathName, pCodeStr );  /* 搜索文件名,*为通配符,适用于所有文件或者子目录 */
...
/*			printf( "Query\n" );
			...
			mStopIfError( i );*/
			printf( "Read\n" );
			if ( CH375vFileSize > FILE_DATA_BUF_LEN ) {  /* 由于演示板用的62256只有32K字节,其中CH375子程序用512字节,所以只读取不超过62个扇区,也就是不超过31744字节 */
				SecCount = FILE_DATA_BUF_LEN / CH375vSectorSize;  /* 由于演示板用的62256只有32K字节,其中CH375子程序用512字节,所以只读取不超过62个扇区,也就是不超过31744字节 */
...
/* 产生新文件 */
		LED_WR_ACT( );  /* 写操作 */
		printf( "Create\n" );
		mCopyCodeStringToXRAM( mCmdParam.Create.mPathName, "\\NEWFILE.TXT" );  /* 新文件名,在根目录下 */
...
/* 删除某文件 */
/*		printf( "Erase\n" );
		mCopyCodeStringToXRAM( mCmdParam.Create.mPathName, "\\OLD" );  将被删除的文件名,在根目录下
