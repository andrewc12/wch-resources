...
/* 已经改名，冲突变量已加后缀_375，可以与CH374的库同时使用，仅剩下DISK_BASE_BUF和FILE_DATA_BUF与CH374的库是合用的，但也可以在.H文件中自行修改 */
...
typedef union _CMD_PARAM {
	...
	} BOC;								/* CMD_BulkOnlyCmd, 执行基于BulkOnly协议的命令, 如果有数据传输那么数据在pDISK_BASE_BUF_375中 */
} CMD_PARAM;
...
#define DISK_BASE_BUF_LEN		512		/* 默认的磁盘数据缓冲区大小为512字节,建议选择为2048甚至4096以支持某些大扇区的U盘,为0则禁止在.H文件中定义缓冲区并由应用程序在pDISK_BASE_BUF_375中指定 */
#endif
...
/* 在CH375子程序读写文件数据时,CH375的程序库提供两种优化速度的方式,在LIB_CFG_FILE_IO中定义:
   ...
   对于文件数据读写,也就是应用程序调用CH375FileReadX和CH375FileWriteX子程序时:
     ...
     例如: 某文件长度为1K(占用2个扇区), 如果调用CH375FileReadX时读1K(指定mCmdParam_375.Read.mSectorCount为2), 那么1K数据全读到指定缓冲区中,
...
#endif
extern PUINT8	pDISK_BASE_BUF_375;			/* 指向外部RAM的磁盘数据缓冲区,缓冲区长度不小于CH375vSectorSize,由应用程序初始化 */
...
/* 子程序库中提供的子程序 */
/* 下述子程序中, 文件操作子程序CH375File*和磁盘查询子程序CH375DiskQuery都可能会用到磁盘数据缓冲区pDISK_BASE_BUF_375,
   并且有可能在pDISK_BASE_BUF_375中保存了磁盘信息, 所以必须保证pDISK_BASE_BUF_375不被用于其它用途,
   如果RAM较少, 要将pDISK_BASE_BUF_375临时用于其它用途, 那么在临时用完后必须调用CH375DirtyBuffer清除磁盘缓冲区 */
...
extern CMD_PARAM_I mCmdParam_375;				/* 命令参数 */
...
#endif
extern void xQueryInterrupt_375( void );		/* 外部定义的被CH375程序库调用的子程序,查询CH375中断并更新中断状态 */
extern void xDelay100uS_375( void );			/* 外部定义的被CH375程序库调用的子程序,延时100uS */
#ifdef EN_DISK_WRITE
extern void xDelayAfterWrite_375( void );		/* 外部定义的被CH375程序库调用的子程序,写操作后延时 */
#endif
extern void xFileNameEnumer_375( void );		/* 外部定义的被CH375程序库调用的子程序,文件名枚举回调子程序 */
...
CMD_PARAM_I mCmdParam_375;					/* 命令参数 */

#if DISK_BASE_BUF_LEN
extern	UINT8  DISK_BASE_BUF[ DISK_BASE_BUF_LEN ];	/* 外部RAM的磁盘数据缓冲区,缓冲区长度为一个扇区的长度,起始地址必须为8字节边界地址 */
#endif
#ifdef FILE_DATA_BUF_LEN
extern	UINT8  FILE_DATA_BUF[ FILE_DATA_BUF_LEN ];	/* 外部RAM的文件数据缓冲区,缓冲区长度不小于一次读写的数据长度,起始地址建议为8字节边界地址 */
UINT8	CH375FileRead( void )		/* 从当前文件读取数据 */
{
	mCmdParam_375.ReadX.mDataBuffer = &FILE_DATA_BUF[0];  /* 指向文件数据缓冲区 */
	return( CH375FileReadX( ) );
}
#ifdef EN_DISK_WRITE
UINT8	CH375FileWrite( void )		/* 向当前文件写入数据 */
{
	mCmdParam_375.WriteX.mDataBuffer = &FILE_DATA_BUF[0];  /* 指向文件数据缓冲区 */
...
UINT8	CH375FileQuery( void )			/* 查询当前文件的信息 */
{
	PUINT8	buf;
	UINT8	count;
	buf = (PUINT8)( & mCmdParam_375.Modify.mFileSize );
	for ( count = sizeof( mCmdParam_375.Modify ); count != 0; count -- ) {
...
void xQueryInterrupt_375( void )			/* 查询CH375中断并更新中断状态 */
{
...
void xQueryInterrupt_375( void )			/* 查询中断状态,等待硬件中断 */
{
...
void xDelay100uS_375( void )				/* 延时100uS */
{
	UINT32	count;
	for ( count = 2500; count != 0; count -- );  /* 延时100uS,2x20nS@50MHz */
}
#endif

#ifdef EN_DISK_WRITE
#ifndef NO_DEFAULT_DELAY_WRITE			/* 在应用程序中定义NO_DEFAULT_DELAY_WRITE可以禁止默认的写操作后延时程序,然后用自行编写的程序代替它 */
void xDelayAfterWrite_375( void )			/* 写操作后延时 */
{
	UINT32	count;
	for ( count = 5000; count != 0; count -- );  /* 延时200uS左右 */
}
#endif
#endif

#ifndef NO_DEFAULT_FILE_ENUMER			/* 在应用程序中定义NO_DEFAULT_FILE_ENUMER可以禁止默认的文件名枚举回调程序,然后用自行编写的程序代替它 */
void xFileNameEnumer_375( void )			/* 文件名枚举回调子程序 */
{
/* 如果指定枚举序号CH375vFileSize为0xFFFFFFFF后调用FileOpen，那么每搜索到一个文件FileOpen都会调用本回调程序，
   回调程序xFileNameEnumer_375返回后，FileOpen递减CH375vFileSize并继续枚举直到搜索不到文件或者目录。建议做法是，
   ...
   如果在本回调程序xFileNameEnumer_375中将CH375vFileSize置为1，那么可以通知FileOpen提前结束搜索。以下是回调程序例子 */
#if 0
#ifdef FILE_DATA_BUF_LEN
	UINT8			i;
	UINT16			FileCount;
	...
	PUINT8			NameBuf;
	pFileDir = (P_FAT_DIR_INFO)( pDISK_BASE_BUF_375 + CH375vFdtOffset );  /* 当前FDT的起始地址 */
...
/*	if ( (UINT32)current_buffer + mLength >= (UINT32)&FILE_DATA_BUF + sizeof( FILE_DATA_BUF ) ) return;*/  /* 防止缓冲区溢出 */
	...
	else {  /* 重试,恢复缓冲区起址,如果将文件数据读写的缓冲区的当前指针放在mCmdParam_375.ReadX.mDataBuffer中则会被自动恢复,无需下面的两行程序 */
		current_buffer += (UINT32)mCmdParam_375.ReadX.mDataBuffer;  /* mDataBuffer中为负值 */
		mCmdParam_375.ReadX.mDataBuffer = 0;  /* 为了支持重试,在调用CH375FileReadX之前也应该清0 */
	}
}
#ifdef EN_DISK_WRITE
void xReadFromExtBuf( UINT8 mLength )	/* 该子程序由CH375的子程序库调用,用于从外部缓冲区读取文件数据到CH375,被CH375FileWriteX调用 */
{
	...
	else {  /* 重试,恢复缓冲区起址,如果将文件数据读写的缓冲区的当前指针放在mCmdParam_375.WriteX.mDataBuffer中则会被自动恢复,无需下面的两行程序 */
		current_buffer += (UINT32)mCmdParam_375.WriteX.mDataBuffer;  /* mDataBuffer中为负值 */
		mCmdParam_375.WriteX.mDataBuffer = 0;  /* 为了支持重试,在调用CH375FileWriteX之前也应该清0 */
	}
}
#endif
#endif
...
#if DISK_BASE_BUF_LEN
	pDISK_BASE_BUF_375 = & DISK_BASE_BUF[0];  /* 指向外部RAM的磁盘数据缓冲区 */
#endif
