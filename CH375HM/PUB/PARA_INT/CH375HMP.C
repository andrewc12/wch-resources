...
ExecCommandStatus:
	...
	if ( status == ERR_SUCCESS ) {  /* 操作成功 */
		...
		i = CH375HM_DATA_RD( );  /* 从索引地址PARA_STS_LEN_ADDR读取返回结果数据的长度 */
		if ( i ) {  /* 有结果数据 */
			CH375HM_INDEX_WR( PARA_BUFFER_ADDR );  /* 指向缓冲区 */
...
//		status = ERR_SUCCESS;
		...
		if ( status == ERR_DISK_DISCON || status == ERR_USB_CONNECT ) {  /* U盘刚刚连接或者断开,应该延时几十毫秒再操作 */
			mDelay100mS( );
			if ( mIntStatus != status ) goto ExecCommandStatus;  /* 如果仍然有中断请求说明之前的中断是U盘插拔通知中断 */
		}
	}
	return( status );
}
...
/* 假定CH375模块的INT#引脚连接到单片机的INT0引脚 */
	IT0 = 1;  /* 置CH375模块中断信号为下降沿触发,实际上,电平触发方式也可以 */
	CH375HM_INT_FLAG = 0;  /* 清中断标志 */
	CH375HM_INT_EN = 1;  /* 允许CH375模块中断 */
/* 其它电路初始化 */
	EA = 1;  /* 初始化完成,开中断 */
	...
	while ( 1 ) {  /* 主循环 */
		printf( "Wait Udisk\n" );
		while ( mIntStatus != ERR_USB_CONNECT );  /* 等待U盘连接,由中断服务程序产生状态 */
		mDelay100mS( );  /* 延时,可选操作,有的USB存储器需要几十毫秒的延时 */
		mDelay100mS( );
		ExecCommand( CMD_QueryStatus, 0 );  /* 查询当前模块的状态,实际上用于撤消模块产生的中断 */
		if ( mCmdParam.Status.mDiskStatus != DISK_CONNECT ) continue;  /* 有时候,USB端口接触不良或者插拔不干脆,导致U盘刚连接又断开 */
		LED_OUT = 0;  /* LED亮 */
...
/* 等待U盘断开 */
		printf( "Take_out\n" );
		while ( mIntStatus != ERR_DISK_DISCON );  /* 等待U盘断开,由中断服务程序产生状态 */
		ExecCommand( CMD_QueryStatus, 0 );  /* 查询当前模块的状态,实际上用于撤消模块产生的中断 */
		LED_OUT = 1;  /* LED灭 */
	}
}
